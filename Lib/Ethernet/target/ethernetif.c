...
{
	if ((HAL_ETH_GetDMAError(handlerEth) & ETH_DMASR_RBUS) ==
	    ETH_DMASR_RBUS) {
		osSemaphoreRelease(RxPktSemaphore);
	}
}
...
/* don't set NETIF_FLAG_ETHARP if this device is not an ethernet one */
	...
	attributes.priority = osPriorityRealtime7 - 1;
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
	...
	if (HAL_ETH_Transmit_IT(&heth, &TxConfig) == HAL_OK) {
		while (osSemaphoreAcquire(TxPktSemaphore,
...
void ethernetif_input(void *argument)
{
	...
	for (;;) {
		if (osSemaphoreAcquire(RxPktSemaphore,
				       TIME_WAITING_FOR_INPUT) == osOK) {
			do {
...
int32_t ETH_PHY_IO_ReadReg(uint32_t DevAddr, uint32_t RegAddr,
			   uint32_t *pRegVal)
{
	if (HAL_ETH_ReadPHYRegister(&heth, DevAddr, RegAddr, pRegVal) !=
	    HAL_OK) {
		return -1;
	}
...
{
	if (HAL_ETH_WritePHYRegister(&heth, DevAddr, RegAddr, RegVal) !=
	    HAL_OK) {
		return -1;
	}
